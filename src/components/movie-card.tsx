 ...
 *
 * On hover the card performs a subtle, fixed 3D tilt — a CSS-only effect (no
 * JS/pointer tracking, stays a server component). The wrapper owns the
 * perspective; the Card is the tilted plane. Everything is gated behind
 * `motion-safe`, so prefers-reduced-motion users get no motion at all.
 */
export function MovieCard({ movie }: { movie: Movie }) {
  return (
    <div className="[perspective:1000px]">
      <Card className="gap-0 ring-white/10 bg-neutral-900/60 py-0 [transform-style:preserve-3d] motion-safe:transition-[transform,box-shadow] motion-safe:duration-300 motion-safe:ease-out motion-safe:will-change-transform motion-safe:hover:[transform:rotateX(5deg)_rotateY(-6deg)_translateZ(14px)_scale(1.03)] motion-safe:hover:shadow-2xl motion-safe:hover:shadow-black/60">
