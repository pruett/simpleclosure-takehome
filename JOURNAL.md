...
- **Testing gotcha**: `bun test` sets `NODE_ENV=test`, so (a) it does NOT auto-load `.env.local`, and (b) the `server-only` package throws on import (no `react-server` condition). Both are handled by the preload `src/test/setup-tests.ts`, wired via `bunfig.toml` `[test].preload`, which stubs `server-only` and loads `.env.local`. Also needed `@types/bun` (dev) so `next build`'s typecheck resolves `bun:test`.
